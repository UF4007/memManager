...
*
****************************************************************************/
#include "example_ble_server_throughput.h"
#include "ble_extern_callback/ble_extern_callback.h"
...
static char* TEST_DEVICE_NAME = NULL;
...
static esp_gatt_rsp_t rsp_read_data;
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        rsp_read_data.attr_value.handle = param->read.handle;
        esp_ble_gatts_send_response(gatts_if, param->read.conn_id, param->read.trans_id,
                                    ESP_GATT_OK, &rsp_read_data);
        break;
    }
    case ESP_GATTS_WRITE_EVT: {
        ESP_LOGI(GATTS_TAG, "GATT_WRITE_EVT, handle %d, value len %d, offset: %d", param->write.handle, param->write.len, param->write.offset);
        //ESP_LOG_BUFFER_HEX(GATTS_TAG, param->write.value, param->write.len);
        ble_server_char_rec_isr((char*)param->write.value, param->write.len, param->write.offset);

        if (param->write.len <= gatts_demo_char1_val.attr_len)
        {
            memcpy(gatts_demo_char1_val.attr_value, param->write.value, param->write.len);
            ESP_LOGI(GATTS_TAG, "data: %s", (char*)param->write.value);
        }
        else
        {
            ESP_LOGE(GATTS_TAG, "Write value length is too long!");
        }
#if (CONFIG_EXAMPLE_GATTS_NOTIFY_THROUGHPUT)
        if (!param->write.is_prep){
...
{
    memset(&rsp_read_data, 0, sizeof(esp_gatt_rsp_t));
    TEST_DEVICE_NAME = deviceName;
    esp_err_t ret;
    ...
    esp_err_t local_mtu_ret = esp_ble_gatt_set_local_mtu(GATTS_DEMO_CHAR_VAL_LEN_MAX);
    if (local_mtu_ret){
...
void server_send_notify(void *notify, unsigned int size)
{
    // esp_ble_gatts_send_indicate(gl_profile_tab[PROFILE_A_APP_ID].gatts_if, gl_profile_tab[PROFILE_A_APP_ID].conn_id,
    //                             gl_profile_tab[PROFILE_A_APP_ID].char_handle,
    //                             sizeof(size), notify, false);

    rsp_read_data.attr_value.len = ESP_GATT_MAX_ATTR_LEN - 1 < size ? ESP_GATT_MAX_ATTR_LEN - 1 : size;
    
    for (int i = 0; i < rsp_read_data.attr_value.len; i++)
    {
        rsp_read_data.attr_value.value[i] = ((uint8_t*)notify)[i];
    }
}