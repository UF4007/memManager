...
class lowlevel {
	...
	struct memPtrComm {
		...
		static std::deque<memPtrComm> memPool;		//allocate only, never free
...
inline std::deque<lowlevel::memPtrComm> lowlevel::memPtrComm::memPool = {};		//enjoy only, never disturbed
inline std::stack<lowlevel::memPtrComm*> lowlevel::memPtrComm::memFreed = {};
inline void* lowlevel::memPtrComm::operator new(size_t size) noexcept
{
	if (memFreed.size() == 0)
	{
		memPool.emplace_back();
		return &*(memPool.end() - 1);
	}
	else
	{
		memPtrComm* ret = memFreed.top();
		memFreed.pop();
		return ret;
	}
}
inline void lowlevel::memPtrComm::operator delete(void* pthis) noexcept
{
	// there are some bugs here in certain scenarios
	//if (std::find(memFreed._Get_container().begin(), memFreed._Get_container().end(), pthis) != memFreed._Get_container().end())
	//	assert(false);
	memFreed.push((memPtrComm*)pthis);
}